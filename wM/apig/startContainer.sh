...
. /opt/softwareag/profiles/IS_$INSTANCE_NAME/bin/apigw_updateNode.sh
JL=""
if [ "$JSON_LOGGING" = true ] ; then JL="-logformat json" ; fi
 . /opt/softwareag/profiles/IS_$INSTANCE_NAME/bin/apigw_updateUIProps.sh
 
if [ $dbgMode == true ]; then
  ${SAG_ROOT_DIR}/profiles/IS_$INSTANCE_NAME/bin/startDebugMode.sh suspend n -log both $JL &
else 
  ${SAG_ROOT_DIR}/profiles/IS_$INSTANCE_NAME/bin/console.sh -log both $JL &
fi
child=$!
wait_term